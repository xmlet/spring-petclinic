...
public class CreateOrUpdateVisitForm {
	...
	static void template(Div<?> container) {
		container.h2()
			...
			.tr()
			.td()
			.<Pet>dynamic((td, pet) -> td.text(pet != null && pet.getName() != null ? pet.getName() : ""))
			.__() // td
			.td()
			.<Pet>dynamic((td, pet) -> td.text(pet != null && pet.getBirthDate() != null ? pet.getBirthDate() : ""))
			.__() // td
			.td()
			.<Pet>dynamic((td, pet) -> td.text(pet != null && pet.getType() != null ? pet.getType() : ""))
			.__() // td
			.td()
			.<Pet>dynamic((td,
					pet) -> td.text(pet != null && pet.getOwner() != null
			...
			.__() // th
			.__() // tr
			.<Pet>dynamic((tbody, pet) -> pet.getVisits()
				.forEach(v -> tbody.tr()
					.td()
					.text(v.getDate())
					.__() // td
