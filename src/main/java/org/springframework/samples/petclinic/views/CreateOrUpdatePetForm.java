...
public class CreateOrUpdatePetForm {
	...
	public void template(Div<?> container) {
		container.h2().<Pet>dynamic((h2, pet) -> {
			if (pet == null)
				h2.text("New ");
		})
			.text("Pet")
			...
			.attrClass("col-sm-10")
			.span()
			.<Pet>dynamic((span, pet) -> span.text(from(pet,
					p -> p.getOwner() != null ? p.getOwner().getFirstName() + " " + p.getOwner().getLastName() : "")))
			.__() // span
			.__() // div
			.__() // div
			.<Pet>dynamic((div, pet) -> partialInputField(div, "Name", "name", from(pet, o -> o.getName())))
			.<Pet>dynamic(
					(div, pet) -> partialInputField(div, "Birth Date", "birthDate", from(pet, p -> p.getBirthDate())))
